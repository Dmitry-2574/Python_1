...
function generateProverb(variants, proverbs) {
    ...
    const randomNewVariant = variants[randomVariantIndex];
  
    const newProverb = randomProverb.replace("Ум", randomNewVariant);
...
let proverbs2 = generateProverb(variants, proverbs)
document.getElementById("result").innerHTML = proverbs2;

function generateMultipleProverbs(count, variants, proverbs) {
  const generateProverbs = [];
  for (let i = 0; i < count; i++) {
  const newProverd2 = generateProverb(variants, proverbs);
  generateProverbs.push(newProverd2);
 
  }
  return generateProverbs;
}

let proverbs3 = generateMultipleProverbs(count, variants, proverbs)
document.getElementById("result2").innerHTML = proverbs3;